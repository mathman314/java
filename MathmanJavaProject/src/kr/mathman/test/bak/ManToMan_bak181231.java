 ...
 *
 */
public class ManToMan_bak181231 {

	static final boolean infoLog = false;	// 상세 로그
...
//		}
		...
		for (i=1; i<=c2Arr.length/2; i++) {
			...
			if (i>1) {
				// 재귀함수 호출
				if (chkMethod(i, 1, 0, bl)) return true;
			} else {
//				print("	check(1)");	// 쌍체크 확인
	 ...
	 * @return
	 */
	private static boolean chkMethod(int i, int c, int j, boolean[] bl) {
//		print("chkMethod("+i+","+c+","+j+")");
		for (int k=j+1; k<c2Arr.length; k++) {
//			print("chkMethod("+i+","+c+","+j+","+k+")");
			bl[k] = true;
			if (i>c+1) {
				if (chkMethod(i, c+1, k, bl)) return true;
			} else {
